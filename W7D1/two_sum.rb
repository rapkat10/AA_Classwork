...
def bad_two_sum?(arr, target_sum)
    ...
    while idx < arr.length
        ...
        while idx_2 < arr.length
            return true if arr[idx] + arr[idx_2] == target_sum
            idx_2 += 1
        end
        idx += 1
    end
...
# Brute force
# If we weren't worried about time complexity, 
# what would be the most straightforward way to solve this problem? 
# To start with, we could check every possible pair. 
# If we sum each element with every other, 
# we're sure to either find the pair that sums to the target, 
# or determine that no such pair exists.

# This is the brute force solution. 
# It's essentially hitting the problem with a sledge hammer. 
# There are faster and more elegant ways to solve the problem, 
...
# and by not checking pairs more than once. However, 
# these micro-optimizations will not improve the time complexity of the solution. 
# Do you see why?)
...
# the brute-force approach is beneath you. Leave that nonsense to the riffraff. 
