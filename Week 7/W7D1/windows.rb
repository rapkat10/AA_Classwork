...
# Calculate max - min and compare it to current_max_range. Reset the value of 
# current_max_range if necessary.
...
def naive_solution(arr, window)
    ...
    while idx2 < arr.length + 1
        if (arr[idx1...idx2].max - arr[idx1...idx2].min) > curr_diff
            curr_diff = arr[idx1...idx2].max - arr[idx1...idx2].min
        end
        idx1 += 1
        idx2 = idx1 + window
    end
...
class StackQueue
    def initialize
        @pushstack1 = MyStack.new
        @popstack2 = MyStack.new
    end

    def size
        @pushstack1.size + @popstack2.size
    end

    def empty?
        @pushstack1.empty? && popstack2.empty?
    end
...
# rather than use an Array, we will implement it using our MyStack class under 
# the hood because pushing and popping from MyStack takes O(1) time. 
# Done properly, we will still have a queue but with the advantages of dequeuing in O(1) time.
...
# use your MinMaxStack (phase 5) to build a MinMaxStackQueue.
    def initialize
        @pushstack1 = MinMaxStack.new
        @popstack2 = MinMaxStack.new
    end
    ...
    def max
        [(@popstack2.max), (@pushstack1.max)].max
    end

    def min
        [(@popstack2.min), (@pushstack1.min)].min
    end
...
# We'll modify it so that we always know the maximum value in the stack. 
# We could write a max method that calls @store.max. 
# However, this requires us to iterate over every item in the @store array, 
# which gives us a time complexity of O(n). This isn't good enough for us; 
# we want to be able to return the max in constant time (O(1)). 
...
# with new element to the store. Think about how to do this and what information to store.
...
# You'll want to follow the same basic approach as above, 
# but use our new data structure instead of array slices. As before, 
# return the current_max_range at the end of the method. 
# Make sure all the test cases pass, and that you both understand the time complexity of this solution.
