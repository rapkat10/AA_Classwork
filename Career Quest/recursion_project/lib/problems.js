...
function fileFinder(directories, targetFile) {
    const topLevel = Object.keys(directories);
    if (topLevel.includes(targetFile)) return true;

    for (let i = 0; i < topLevel.length; i++) {
        let el = topLevel[i];
        if (el[0] === "/") {
            const value = fileFinder(directories[el], targetFile);
...
function pathFinder(directories, targetFile) {
    const topLevel = Object.keys(directories);
    if (topLevel.includes(targetFile)) {
        return `/${targetFile}`;
    }
    ...
    for (let i = 0; i < topLevel.length; i++) {
        let el = topLevel[i];
        if (el[0] === "/") {
            const value = pathFinder(directories[el], targetFile);
            if (value) return el.concat(value);
        }
    }
    return null;
}
