...
function stepper(nums, memo = {}) {
    ...
    //     const stepable = steps.some((e) => nums[i + e] !== 0) 
...
function maxNonAdjacentSum(nums, memo = {}) {
    ...
    let first = nums[0];
    memo[nums.length] = Math.max( first + maxNonAdjacentSum(nums.slice(2), memo), 
...
function minChange(coins, amount) {
    ...
    for (let i = 1; i <= amount; i++) {
        for (let j = 0; j < coins.length; j++) {
            if (coins[j] <= i) {
                let result = Math.min(table[i], table[i - coins[j]] + 1);
                table[i] = result;
            }
        }
    }
